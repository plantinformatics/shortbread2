...
def group_features(db):
    ...
    for feature in db.all_features():
        ...
        'repeat_region', 'regulatory', 'stem_loop', 'gene_component_region', 'repeat_region'):
...
def print_features(feature_groups):
    for gene_id in feature_groups:
        ...
        print(feature_group.gene.chrom, 'EMBL', 'gene', feature_group.gene.start, feature_group.gene.stop, '.',
              feature_group.gene.strand, '.', gene_attributes, sep='\t')

        transcript_attributes = f'ID=transcript:{gene_id};Parent=gene:{gene_id};Name={name};biotype=protein_coding;transcript_id={gene_id}'
        print(feature_group.transcript.chrom, 'EMBL', 'transcript', feature_group.transcript.start,
              feature_group.transcript.stop, '.', feature_group.transcript.strand, '.', transcript_attributes, sep='\t')
...
def main():
    ...
    db = gffutils.create_db(fn, dbfn=db_handle, force=True, keep_order=True, merge_strategy='merge',
